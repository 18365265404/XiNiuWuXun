  ...
  <div class="shopCar">
    <div class="layout car-box">
        ...
        <div class="tb-body" v-for="(item,index) in carList" :key="index">
            <div class="body-check body-des">
                <Checkbox @on-change='checkedfn' v-model="item.checked"></Checkbox>
                <img @click="goDetail(item.goodsId)" class="cursor" style="width:80px;" :src="qiniuHead+item.imgUrl" alt="">
            </div>
            <div class="body-msg body-des">
                
                <h2 @click="goDetail(item.goodsId)" class="cursor">{{item.goodsName}}</h2>
            </div>
...
methods:{
    ...
    deletMost(){
        ...
        if(strsArr.length==0){
            this.$Message.warning('至少选择一条才可以删除');
            return
        }

        this.statistics()
        let userDate = session_storage.storageGet('userData')
        deleteRequest(url.goodsShoppingCartDeleteShoppingCart+`${userDate.userId}`+'/'+strsArr, {
    ...
    statistics(){
        ...
        session_storage.storageSet('shop-car',this.carList)
    ...
    delet(index){
        ...
        this.statistics()
        let userDate = session_storage.storageGet('userData')
        deleteRequest(url.goodsShoppingCartDeleteShoppingCart+`${userDate.userId}`+'/'+this.carList[index].goodsId+'_'+this.carList[index].specificationsId, {
            // strs:[this.carList[index].goodsId+'_'+this.carList[index].specificationsId]
...
created(){
    ...
    //     return
    // }
    // this.carList =session_storage.storageGet("shop-car") || []
    // this.checkAll=true
    // this.carList.forEach((v,i)=>{
    //     if(!v.checked){
    //         this.checkAll=false

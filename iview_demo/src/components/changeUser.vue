...
export default {
  ...
  computed: {
    ...
    methods: {
        ...
        handleSubmit (name) {
            console.log("111111111111",session_storage.storageGet('userId') )
            this.$refs[name].validate((valid) => {
                if (valid) {
                    // alert()
                    
                    console.log("session_storage.storageGet('userId')",session_storage.storageGet('userId'))
                    putRequest(url.userUserUpdateUserById, {
                        nickname:this.formCustom.nickname,
                        tel:this.formCustom.tel,
                        userId:session_storage.storageGet('userId')
                        ...
                        session_storage.storageSet('userData',userData)

      ...
      <div class="shop-show layout">
          <div class="show-left">
              ...
              <ul class="img-small-box">
                  <li @mouseover="mouseOver(index)" v-for="(item,index) in imglist" :key="index"><img :src="qiniuHead+item.imgUrl" alt=""></li>
...
export default {
  ...
  methods:{
    ...
    mouseOver(index){
        console.log(index)
        this.showImg=this.imglist[index].imgUrl
    },
    ...
    initData(params){
        getRequest(url.goodsGoodsSelectGoodsDetailsByGoodsId+`${params}`).then(data=> {
            ...
            let goodsImgVOList=data.data.data.goodsDetailsVO.goodsImgVOList
            goodsImgVOList.forEach((v,i) => {
                if(v.isDiagram==1){
                    this.shopDes.mainImg=v.imgUrl
                }
            });
            // 商品底部图片详情

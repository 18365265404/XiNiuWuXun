<template>
    <div class="billAddress">
        ...
        <div class="add-box">
            ...
            <span class="add" v-show="formStatus=='add' ? true : false">新增发票寄送地址：</span>
            <span class="change" v-show="formStatus=='change' ? true : false">修改发票寄送地址：</span>
        ...
        <div>
             <Button type="primary" @click="goAdd">新增发票地址</Button>
        </div>
        <table border="1" >
			<tr class="table-head">
                <th>&nbsp;</th>
            ...
            <tr class="table-item" v-for="(item,index) in tableList" :key="item.id">
                ...
                <td>{{item.tel}}</td>
                <td>
                <Button @click="goChange(tableList,index,item.deliveryAddressId)" type="info" ghost>修改</Button>
                <Button @click="delet(tableList,index,item.deliveryAddressId)" type="warning" ghost>删除</Button>
                </td>
                <td>
                    <i-switch  @on-change="changeMoRen(item.isDft,item.deliveryAddressId)" class="form-item-right" v-model="item.isDft==1? true:false" size="large">
...
methods:{
    changeMoRen(val,deliveryAddressId){
      
      putRequest(url.userDeliveryAddressUpdateDeliveryAddressById, {
        // consignee:'',
        // tel:'',
        // provinceName:'',
        // cityName:'',
    ...
    delet(tableList,index,id){
        deleteRequest(url.userDeliveryAddressDeleteDeliveryAddressById+id, {
    ...
    addAddress(city1,city2,city3){
        postRequest(url.userDeliveryAddressInsertDeliveryAddress, {
    ...
    changeAddress(city1,city2,city3){
        ...
        putRequest(url.userDeliveryAddressUpdateDeliveryAddressById, {
            ...
            postalCode:this.form.postalCode,
            isDft:this.form.isDft ? 1 : 0,
            deliveryAddressId:this.changeId
        }).then(data=> {
    ...
    initAddress(){
      ...
      getRequest(url.userDeliveryAddressSelectDeliveryAddressList+`${userDate.userId}`, {
